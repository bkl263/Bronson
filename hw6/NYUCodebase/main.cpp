...
public:
	Player() {}
	Player(float _x, float _y, float tileSize, int t_ind, int SPRITE_COUNT_X, int SPRITE_COUNT_Y) : Entity(_x, _y, tileSize, t_ind, SPRITE_COUNT_X, SPRITE_COUNT_Y) { rocketboost = Mix_LoadWAV("rocketboost.wav"); }
	...
	void boost() {
		Mix_PlayChannel(0, rocketboost, 0);
		y_vel += accel;
	}

private:
	Mix_Chunk *rocketboost;
...
void setup() {
	...
	Mix_OpenAudio(44100, MIX_DEFAULT_FORMAT, 2, 4096);
	Mix_Chunk* start;
	start = Mix_LoadWAV("startup.wav");
	Mix_VolumeChunk(start, 1);
